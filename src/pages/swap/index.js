...
@connect(({ user, pair, loading }) => {
    ...
    return {
        ...
        submiting: effects['pair/reqSwap'] || effects['pair/swap'] || effects['user/transferBsv'] || effects['user/transferFtTres'] || false
    }
...
export default class Swap extends Component {
    ...
    setOriginBalance = () => {
        ...
        const origin_amount = this.state.dirForward ? userBalance.BSV || 0 : userBalance[token2.codeHash] || 0;;
        this.formRef.current.setFieldsValue({
            origin_amount,
        });
        this.setState({
    ...
    calcAmount = (originAddAmount = 0, aimAddAmount = 0) => {
        ...
        let _aimAddAmount = BigNumber(aimAddAmount).multipliedBy(Math.pow(10, token2.decimal));
        let newAmount1 = BigNumber(amount1), newAmount2 = BigNumber(amount2);
        if (originAddAmount > 0) {
            ...
            removeAmount = formatAmount(removeAmount.div(Math.pow(10, token2.decimal)), 8);
            ...
            <Spin spinning={submiting}>
                <Form onSubmit={this.handleSubmit} ref={this.formRef}>
                    <div className={styles.title}>
                        <h3>{_('you_pay')}</h3>
                        <div className={styles.balance} onClick={this.setOriginBalance}>{_('your_balance')}: <span>{userBalance[origin_token.codeHash || 'BSV'] || 0}  {symbol1}</span></div>
                    </div>
    ...
    renderButton() {
        ...
        else if (BigNumber(aim_amount).multipliedBy(Math.pow(10, aim_token.decimal)).isGreaterThan(dirForward ? pairData.swapToken2Amount : pairData.swapToken1Amount)) {
            // 池中币不足
    ...
    submit = async () => {
        ...
        if (dirForward) {
            ...
            const amount = BigNumber(origin_amount).multipliedBy(Math.pow(10, token2.decimal)).toNumber()
    ...
    renderResult() {
        const { showDetail, origin_amount, aim_amount, fee, dirForward, txid } = this.state;
        ...
        return <div className={styles.content}>
            ...
            <div className={styles.finish_title}>{_('swapping_for').replace('%1', symbol1).replace('%2', symbol2)}</div>
            ...
            {showDetail ? <div className={styles.detail}>
                ...
                <div className={styles.detail_item}>
                    ...
                    <div className={styles.item_value}>{origin_amount} {symbol1}</div>
                </div>
                <div className={styles.detail_item}>
                    <div className={styles.item_label}>{_('received')}</div>
                    <div className={styles.item_value}>{aim_amount} {symbol2}</div>
                </div>
                <div className={styles.detail_item}>
                    <div className={styles.item_label}>{_('swap_fee')}</div>
                    <div className={styles.item_value}>{fee} {symbol1}</div>
                </div>
                <div className={styles.detail_item}>
            ...
            <Button className={styles.done_btn} onClick={this.finish}>{_('done')}</Button>
        </div>
    }
    finish = () => {
        this.setState({
